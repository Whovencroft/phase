...
///
/// Uses nom `tag()` combinators at each word boundary — the same
/// word-boundary scanning pattern used by `scan_timing_restrictions`.
fn find_verb_boundary_in_named(name_text: &str) -> Option<usize> {
    ...
    let is_conjugated_clause_verb = |word_start: &str| -> bool {
        alt((
    ...
    let mut offset = 0;
    while offset < name_text.len() {
        if let Some(space_pos) = name_text[offset..].find(' ') {
            let abs_space = offset + space_pos;
            let word_start = &name_text[abs_space + 1..];
            if !word_start.is_empty() && is_conjugated_clause_verb(word_start) {
                return Some(abs_space);
            }
            offset = abs_space + 1;
        } else {
            break;
